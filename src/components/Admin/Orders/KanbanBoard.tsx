      ...
      {stages.map((stage) => {
        ...
        return (
          <div key={stage} className="bg-stone-50 dark:bg-stone-900 border border-stone-200/55 dark:border-stone-800 p-4 rounded-2xl flex flex-col min-h-[480px]">
            ...
            <div className="flex justify-between items-center pb-3 border-b border-stone-200 dark:border-stone-800 mb-4 shrink-0 text-left">
              <span className="text-xs font-semibold text-stone-900 dark:text-stone-300">
                {stage}
              </span>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[420px]">
              {stageOrders.map((ord) => (
                <div
                  key={ord.id}
                  className="bg-white dark:bg-stone-800 p-4 rounded-xl border border-orange-100/40 dark:border-stone-800 hover:border-orange-200 dark:hover:border-stone-700 shadow-sm cursor-pointer space-y-3 text-left transition-all duration-200"
                >
                  <div className="flex justify-between items-start" onClick={() => onSelectOrder(ord)}>
                    ...
                    <span className="font-bold text-xs text-stone-800 dark:text-stone-100 shrink-0">
                      ₹{ord.amount}
                    </span>
                  </div>
                  ...
                  {/* Rider panel display */}
                  <div className="flex justify-between items-center text-[10px] font-bold border-t border-stone-200 dark:border-stone-800 pt-2.5">
                    <div className="flex items-center gap-1 text-stone-500">
                      <Truck className="w-3.5 h-3.5" />
                      <span>{ord.partner ? ord.partner : 'Unassigned'}</span>
                    </div>
                      ...
                      <div className="flex gap-1.5 shrink-0">
                        {!ord.partner && (
                          <button
                            ...
                            className="text-[9px] bg-orange-50 hover:bg-orange-100 text-orange-600 px-2 py-0.5 rounded font-bold transition cursor-pointer"
                          >
                            Assign
                          </button>
                        )}
                        <button
                          ...
                          className="text-[9px] bg-green-50 hover:bg-green-100 text-green-700 px-2 py-0.5 rounded font-semibold transition flex items-center gap-0.5 cursor-pointer"
                        >
                          <span>Next</span>
