...
export const OrderDetailsDrawer: React.FC<OrderDetailsDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white dark:bg-stone-800 p-6 sm:p-8 shadow-2xl overflow-y-auto border-l border-orange-100 dark:border-stone-700 animate-in slide-in-from-right duration-250 text-left text-neutral-700 dark:text-neutral-200">
      <button
        onClick={onClose}
      ...
      <div className="space-y-6">
        <div className="border-b pb-4 dark:border-stone-800">
          <span className="text-xs font-medium text-stone-500 block">Order details</span>
          <h3 className="text-xl font-semibold text-stone-900 dark:text-white mt-1">
            {order.id}
          </h3>
        </div>
        ...
        <div className="space-y-4">
          <h5 className="font-semibold text-stone-900 dark:text-white border-b pb-2 text-sm">Store and items</h5>
          <div className="text-sm font-semibold text-stone-800 dark:text-white">
            Kitchen outlet: <span className="text-orange-600">{order.restaurant}</span>
          </div>
        ...
        <div className="space-y-2 text-xs font-semibold">
          <h5 className="font-semibold text-stone-500 text-sm">Workflow</h5>
          <div className="flex gap-2">
            <div className="flex-1 bg-stone-100 dark:bg-stone-900 p-2 text-center rounded">
              ...
              <b className="block text-orange-600 font-extrabold mt-1 text-xs">{order.status}</b>
            </div>
            <div className="flex-1 bg-stone-100 dark:bg-stone-900 p-2 text-center rounded">
              <span className="text-[10px] text-stone-400">Rider</span>
        ...
        <div className="pt-6 border-t dark:border-stone-800 flex gap-3">
          <button
            ...
            className="flex-1 py-3 bg-stone-100 hover:bg-stone-200 dark:bg-stone-800 dark:hover:bg-stone-700 text-stone-700 dark:text-stone-300 font-bold text-xs rounded-xl transition-all cursor-pointer"
          >
            Notify customer
          </button>
          {order.status !== 'Delivered' && (
            <button
              ...
              className="flex-1 py-3 bg-orange-600 hover:bg-orange-700 text-white font-extrabold text-xs rounded-xl transition-all cursor-pointer"
            >
              Advance stage
            </button>
          )}
